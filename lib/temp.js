...
function mkdirp(affixes, callback) {
  var dirPath = generateName(affixes, 'd-');
  mkdirpFS(dirPath, {"mode":0700}, function(err) {
    if (!err) {
      deleteDirOnExit(dirPath);
    }
    if (callback) {
...
function mkdirpSync(affixes) {
  var dirPath = generateName(affixes, 'd-');
  mkdirpFS.sync(dirPath, {"mode":0700});
  deleteDirOnExit(dirPath);
  return dirPath;
}
